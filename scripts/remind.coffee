...
#
# Commands:
#   hubot recuerdame en <time> <action> - Establece un recordatorio
#
# Author:
#   whitman
    ...
    if @cache.length > 0
      ...
      if @cache.length > 0
        trigger = =>
          ...
          @robot.reply reminder.msg_envelope, 'me pediste que te recordara ' + reminder.action
          @queue()
...
module.exports = (robot) ->
  ...
  robot.respond /recuerdame en ((?:(?:\d+) (?:semanas?|dias?|horas?|hrs?|minutos?|mins?|segundos?|secs?)[ ,]*(?:y)? +)+)(.*)/i, (msg) ->
    time = msg.match[1]
    action = msg.match[2]
    reminder = new Reminder msg.envelope, time, action
    reminders.add reminder
    msg.send 'Recordatorio para ' + reminder.dueDate() + ': ' + action
